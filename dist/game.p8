...
end io=ch.fu[ch.im] else io=ch.idle[ik] end hy(ch) local fm=mid(room_curr.min_autoscale or 0,(ch.y+12)/64,1) fm*=(room_curr.autoscale_zoom or 1) local fo=ch.fo or fm local ip=(8*ch.h) local iq=(8*ch.w) local ir=ip-(ip*fo) local is=iq-(iq*fo) ij(io,ch.de+flr(is/2),ch.hv+ir,ch.w,ch.h,ch.trans_col,ch.flip,false,fo) if er
